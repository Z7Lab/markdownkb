...
6. **Compile.** Dense unstructured sources — papers, transcripts, vendor docs — get pointed at the `wiki_compile` plugin. Wikis are named, managed targets the plugin creates and tracks (one per topic or domain — research, work, hobby, a specific project). Each ingest reads the source, synthesizes a concise summary page into the chosen wiki, updates that wiki's index, appends to its chronological log, and notes overlaps with related material already in the wiki. Compiled pages are ordinary markdown and immediately flow through the rest of the pipeline (indexed, searchable, retrievable by chat and the doc map, exposed over MCP). Raw sources stay read-only and untouched. See [wiki-compile.md](../explanation/wiki-compile.md) for the pattern and its relationship to Karpathy's LLM Wiki gist.
...
- **Temp buckets.** Create a temporary collection from arbitrary directories, search it in isolation, delete it when done. Each bucket gets its own ChromaDB collection. Use cases: tutorial transcripts, GitHub `llms.txt` files, conference notes, investigation logs — anything you need searchable temporarily without polluting the permanent knowledge base. Clip multiple YouTube videos, articles, and PDFs into a bucket and chat with them all at once — the response synthesizes across all sources simultaneously. YouTube lets you chat with one video. Buckets let you chat with five, and get answers none of them could give individually. Activate a bucket alongside a scope and retrieval draws from **both** — your permanent curated knowledge base and the bucket's temporary content — merged and ranked in a single response. The LLM sees context from both simultaneously, so you can compare new material against existing knowledge, bridge a gap, or ask what's changed — without moving anything into your permanent index.
