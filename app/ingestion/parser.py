...
def parse_markdown(filepath: str, source_root: str = "") -> list[Chunk]:
...
def parse_and_chunk(filepath: str, source_root: str = "",
                    max_chunk_size: int = 512, overlap: int = 50) -> list[Chunk]:
    raw_chunks = parse_markdown(filepath, source_root)
    result: list[Chunk] = []
    global_idx = 0

    for chunk in raw_chunks:
        sub_texts = chunk_text(chunk.content, max_chunk_size, overlap)
