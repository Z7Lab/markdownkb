...
import { cn, relativeTime, slugifyFilename } from "@/lib/utils"
...
import {
  ...
  Infinity as InfinityIcon, RefreshCw, Download, FolderInput, Github, FilePlus,
  EyeOff, Eye,
} from "lucide-react"
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip"
import { BucketEditForm } from "./bucket-edit-form"
import { BucketPathStatus } from "./bucket-path-status"
import { BucketChatSection } from "./bucket-chat-section"
...
/**
 * A small horizontal tab button. We deliberately avoid the shared Radix `Tabs`
 * here: the whole app is wrapped in a vertical `Tabs` (App.tsx), and shadcn's
 * tab styles key off a shared `group/tabs` name, so a nested Tabs inherits the
 * ancestor's vertical orientation and stacks. This local control sidesteps that.
 */
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active && "after:absolute after:inset-x-0 after:-bottom-px after:h-0.5 after:bg-foreground",
      )}
    >
      {children}
    </button>
...
export function BucketDetailPanel({
  ...
  return (
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 pt-5 pb-0 space-y-3 shrink-0">
        {/* Name row */}
